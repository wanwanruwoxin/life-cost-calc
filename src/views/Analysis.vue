...
const averagePerRecord = computed(() => {
  return records.value.length > 0
    ? totalMonthly.value / records.value.length
    : 0;
});
...
const topExpense = computed(() => {
  return records.value.reduce(
    (max, record) => (record.monthlyPrice > max.monthlyPrice ? record : max),
  ...
  <div class="w-full max-w-md mx-auto p-4">
        ...
        <QCardSection class="bg-gradient-to-br from-purple-50 to-pink-100 rounded-t-4">
          <div class="flex items-center">
            ...
            <div>
              ...
              <div class="text-2xl font-bold text-gray-800">
                {{ formatCurrency(averagePerRecord) }}
              </div>
            </div>
          </div>
        </QCardSection>
    ...
    <div class="space-y-6">
      ...
      <QCard class="rounded-4 shadow-lg backdrop-blur-sm bg-white/95">
        ...
        <QCardSection>
          <div class="space-y-4">
            <div v-for="stat in frequencyStats" :key="stat.frequency"
              class="flex items-center justify-between p-3 bg-gray-50 rounded-2">
              <div class="flex items-center">
                <QBadge :color="getFrequencyColor(stat.frequency)" :label="getFrequencyLabel(stat.frequency)"
                  class="mr-3" />
            ...
            <div class="space-y-2">
              <div class="flex justify-between">
                ...
                <span class="font-semibold">{{
                  formatCurrency(topExpense.price)
                  }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">月支出：</span>
                <span class="font-semibold text-orange-600">{{
                  formatCurrency(topExpense.monthlyPrice)
                  }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">年支出：</span>
                <span class="font-semibold text-red-600">{{
                  formatCurrency(topExpense.yearlyPrice)
                  }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">频率：</span>
                <QBadge :color="getFrequencyColor(topExpense.frequency)"
    ...
    <QCard class="rounded-4 shadow-lg backdrop-blur-sm bg-white/95 mt-8">
      ...
      <QCardSection class="p-0">
        <div class="space-y-3 p-4">
          <div v-for="record in records" :key="record.id" class="bg-gray-50 rounded-2 p-3 border border-gray-200">
            <div class="flex items-center justify-between mb-2">
              <h4 class="font-medium text-gray-800">
                {{ record.description }}
              </h4>
              <QBadge :color="getFrequencyColor(record.frequency)" :label="getFrequencyLabel(record.frequency)" />
            </div>
